...
- **폼 제출 시 포함 여부:**
  - `name`이 설정된 입력만 제출 페이로드에 포함된다.
  - `disabled`는 제출 대상에서 제외되며, `readOnly`는 값 제출은 허용하되 편집만 차단한다.
  - `required`는 브라우저 네이티브 유효성 검사를 활용하며, 제출 시 빈 값이면 기본 검증 오류를 발생시킨다.
- **폼 reset 대응:**
  - 컴포넌트는 React 상태로 값을 제어하므로, 네이티브 `form.reset()` 호출만으로는 값이 초기화되지 않는다.
  - reset 시 초기값으로 되돌리려면 상위에서 `onReset` 핸들러를 통해 `value` 상태를 재설정하거나, `defaultValue` 기반 내부 상태를 재마운트하는 패턴을 사용한다.
- **`type="number"` 사용 주의:**
  - 입력 값은 문자열로 유지하며, 로케일별 소수점(`,`/`.`) 처리 차이는 상위 검증 로직에서 처리해야 한다.
  - 마우스 스크롤/키보드 방향키가 값 변경을 일으킬 수 있으므로 숫자 필드를 스크롤 영역에 배치할 때 의도치 않은 값 변경을 대비한다.
