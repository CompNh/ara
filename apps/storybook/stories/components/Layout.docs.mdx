...
`direction`은 표준 flex-direction 그대로 동작합니다. `row`는 인라인 축을 따라 좌→우(또는 RTL에서는 우→좌)로, `column`은 블록 축을 따라 위→
아래로 정렬됩니다. 예상한 방향과 다르게 보이면 `writing-mode` 또는 RTL 여부를 확인해주세요.
</Description>
