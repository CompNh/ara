import { Meta, Title, Subtitle, Description, Canvas, Primary, Controls, ArgsTable, Story } from "@storybook/blocks";
...
가장 직관적인 축 지정 방법은 `orientation` prop을 사용하는 것입니다. `horizontal`/`vertical`(또는 `horizontal-reverse`/`vertical-reverse`)만 전달하면
가로/세로 흐름을 바로 지정할 수 있습니다. 기존 Flexbox 명칭(`row`, `column`)도 `direction` prop으로 그대로 쓸 수 있지만, 기본 안내는 `orientation`
값을 기준으로 제공합니다.
</Description>
...
<Story of={LayoutStories.Playground}>
  <Primary />
  <Controls />
</Story>
...
<Story of={LayoutStories.FlexPlayground}>
  <Canvas />
  <Controls />
</Story>
...
<Story of={LayoutStories.GridPlayground}>
  <Canvas />
  <Controls />
</Story>
...
<Story of={LayoutStories.SpacerPlayground}>
  <Canvas />
  <Controls />
</Story>
