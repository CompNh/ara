...
- **스케일 정의**
  ...
  | 토큰 | 값 | 용도 |
  | --- | --- | --- |
  | `zIndex.base` | 0 | 기본 레이아웃, 배경 콘텐츠 |
  | `zIndex.dropdown` | 1000 | 셀렉트 드롭다운, 컨텍스트 메뉴처럼 뷰포트 기준 포지셔닝 |
  | `zIndex.sticky` | 1020 | 앱 바/스티키 헤더·툴바 |
  | `zIndex.popover` | 1100 | 앵커 정렬이 필요한 팝오버·DatePicker 등 |
  | `zIndex.overlay` | 1200 | 스크림/백드롭(모달·드로어 등) |
  | `zIndex.modal` | 1210 | 모달/드로어 콘텐츠(스크림 바로 위) |
  | `zIndex.toast` | 1250 | 비차단 토스트/알림 컨테이너 |
  | `zIndex.tooltip` | 1300 | 툴팁(최상위, 다른 오버레이 위로 표시) |

- **사용 지침**
  - 드롭다운·팝오버는 서로 겹칠 수 있으므로 `dropdown` < `popover` 순서를 유지한다.
  - 모달 스택은 `overlay`(스크림) + `modal`(내용) 조합으로 사용하고, 팝오버/툴팁이 모달 내부에서 뜰 수 있도록 더 높은 값을 제공한다.
  - 토스트는 모달·오버레이와 동시에 존재할 수 있으므로 별도 레이어(`toast`)를 사용해 화면 구석에서 겹치지 않도록 한다.
...
- **훅**: `useScrollLock({ active = true, ownerDocument? })`
  - body 스크롤을 막고 z-index 스택과 독립적으로 동작한다.
  - 활성화 시 `overflow: hidden` + `overscroll-behavior: none` 으로 바디 스크롤을 차단하고, 뷰포트 스크롤바 너비만큼 패딩을 보정해 레이아웃 점프를 막는다.
  - `touchmove` 이벤트를 캡처해 `body`·`html`에서 시작된 터치 스크롤만 `preventDefault()`로 막아 모바일 배경 스크롤을 방지하되, 오버레이 내부 터치는 허용한다.
  - 여러 오버레이에서 동시에 호출하면 전역 카운터로 관리하며, **마지막 락이 해제될 때**에만 스타일과 이벤트 리스너를 원복한다.
